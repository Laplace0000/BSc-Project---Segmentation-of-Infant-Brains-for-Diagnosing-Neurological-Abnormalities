...
class ColorNames:
    ...
    @staticmethod
    def find_nearest_webcolor_name(rgb_tuple, rev_dict=False):
        return ColorNames.find_nearest_color_name(rgb_tuple, ColorNames.WebColorMap, rev_dict)

    @staticmethod
    def find_nearest_image_magick_color_name(rgb_tuple, rev_dict=False):
        return ColorNames.find_nearest_color_name(rgb_tuple, ColorNames.ImageMagickColorMap, rev_dict)

    @staticmethod
    def find_nearest_color_name(rgb_tuple, Map, reverse_dict=False):
        mindiff = None
        for d in Map:
            r, g, b = ColorNames.rgb_from_str(Map[d])
            diff = abs(rgb_tuple[0] - r) * 256 + abs(rgb_tuple[1] - g) * 256 + abs(rgb_tuple[2] - b) * 256
            if mindiff is None or diff < mindiff:
                mindiff = diff
                mincolorname = d
        if reverse_dict:
            mincolorname = Map[mincolorname]
        return mincolorname.lower()
...
def label2rgbfromlut(lut_str):
    fs_lut = read_classes_from_lut(lut_str)
    label_to_color = fs_lut[["R", "G", "B"]].values / 255
    return label_to_color


def map_grid_to_colormap(colormap, image_grid):
    img = image_grid.numpy().flatten()
    result = colormap[img.astype(int)]
    return result.reshape(image_grid.shape + (3,))
