...
class FastSurferCNNBase(nn.Module):
    ...
    * Loss Function (weighted Cross-Entropy and dice loss)
    ...
    def forward(self, x, scale_factor=None, affine=None, inverse=None, scale_factor_out=None):
        """
        Computational graph
...
class FlexSkipFastSurferCNN(FastSurferCNNBase):
    ...
    * Loss Function (weighted Cross-Entropy and dice loss)
    ...
    def __init__(self, params, padded_size):
        ...
        self.interpol1 = il.Affine2d((self.width, self.height),
                                     interpolation_mode=self.interpolation_mode,
                                     crop_position=self.crop_position,
                                     device=params["device"])

        self.interpol2 = il.Affine2d(self.out_tensor_shape,
                                     interpolation_mode=self.interpolation_mode,
                                     crop_position=self.crop_position,
                                     device=params["device"])
    ...
    def forward(self, x, scale_factor, affine=None, inverse=None, scale_factor_out=None):
        """
        Computational graph
        :param tensor x: input image
        :return tensor: prediction logits
        ...
        encoder_output0, rescale_factor = self.interpol1(skip_encoder_0,
                                                         scale_factor, affine)  # instead of maxpool = encoder_output_0
        ...
        decoder_output0, sf = self.interpol2(decoder_output1, scale_factor_out, inverse, rescale=True)
...
_MODELS = {
    "FastSurferVINN": FlexSkipFastSurferCNN
}


def build_model(cfg, train=False, device="cpu"):
    assert (cfg.MODEL.MODEL_NAME in _MODELS.keys()), \
            f"Model {cfg.MODEL.MODEL_NAME} not supported"
    print(f"Running Model {cfg.MODEL.MODEL_NAME}")
